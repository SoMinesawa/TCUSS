...
python test_SemanticKITTI.py \
    ...
    --sp_path "$SP_PATH" \
    --save_path "$MODEL_DIR" \
    $DEBUG || { echo "テスト実行に失敗しました"; exit 1; }

echo "2. 評価: 公式評価スクリプトで精度計測"
CURRENT_DIR=$(pwd)
cd ~/repos/semantic-kitti-api || { echo "semantic-kitti-apiディレクトリへの移動に失敗しました"; exit 1; }
python evaluate_semantics.py -d "$DATASET_PATH" -p "$MODEL_DIR/pred_result/" || { echo "評価に失敗しました"; exit 1; }
...
cd "$CURRENT_DIR" || { echo "元のディレクトリへの移動に失敗しました"; exit 1; }
python ~/repos/semantic-kitti-api/validate_submission.py "$RESULT_DIR/$SUBMISSION_NAME" "$DATASET_PATH" || { echo "提出ファイル検証に失敗しました"; exit 1; }
